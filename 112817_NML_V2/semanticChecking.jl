...
function semanticCheckingForEachTriplet(tripletArr::Array, conversionDict::Dict)
  preIRForm = []  # array of tuple (verb, reactant, product, catalyst)
  sys2userDict = Dict()
  ...
  for tp in tripletArr
    tmpIR = []
    if tp[1] == "react"
      tmpIR = checkReactType(tp, conversionDict)
    elseif tp[1] == "uptake"
      tmpIR = checkUptakeType(tp, conversionDict, sys2userDict)
    elseif tp[1] == "secrete"
      tmpIR = checkSecreteType(tp, conversionDict, sys2userDict)
    elseif tp[1] == "bind"
      tmpIR = checkBindType(tp, conversionDict)
    elseif tp[1] == "unbind"
      tmpIR = checkUnbindType(tp, conversionDict)
    elseif tp[1] == "phosphorylate"
      tmpIR = checkPhosphorylateType(tp, conversionDict, sys2userDict)
    elseif tp[1] == "dephosphorylate"
      tmpIR = checkDephosphorylateType(tp, conversionDict, sys2userDict)
    elseif tp[1] == "catalyze"
      tmpIR = checkCatalyzeType(tp, conversionDict)
    elseif tp[1] == "induce"  # GENE
      tmpIR = checkInduceType(tp, conversionDict)
    elseif tp[1] == "activate"  # PROTEIN
      tmpIR = checkInduceType(tp, conversionDict)
    elseif tp[1] == "repress"  # GENE
      tmpIR = checkRepressType(tp, conversionDict)
    elseif tp[1] == "inhibit"  # PROTEIN
      tmpIR = checkRepressType(tp, conversionDict)
    else
...
BA
=#
function checkDephosphorylateType(senInfo::Tuple, u2sDict::Dict, s2uDict::Dict)
...
BA
=#
function checkPhosphorylateType(senInfo::Tuple, u2sDict::Dict, s2uDict::Dict)
  legalTypeSet1 = Set(["PROTEIN", "[]"])
  legalTypeSet2 = Set(["PROTEIN"])
  legalTypeSet3 = Set(["SITE"])
  # check parameter setting
...
BA
=#
function checkCatalyzeType(senInfo::Tuple, u2sDict::Dict)
  legalTypeSet1 = Set(["PROTEIN", "[]"])
...
BA
=#
function checkRepressType(senInfo::Tuple, u2sDict::Dict)
  legalTypeSet1 = Set(["PROTEIN", "METABOLITE"])
  # Induce or Activate
  if senInfo[1] == "repress"
...
BA
=#
function checkInduceType(senInfo::Tuple, u2sDict::Dict)
  legalTypeSet1 = Set(["PROTEIN", "METABOLITE"])
  # Induce or Activate
  if senInfo[1] == "induce"
    legalTypeSet2 = Set(["GENE"])
...
BA
=#
function checkUnbindType(senInfo::Tuple, u2sDict::Dict)
...
BA
=#
function checkBindType(senInfo::Tuple, u2sDict::Dict)
...
BA
=#
function checkSecreteType(senInfo::Tuple, u2sDict::Dict, s2uDict::Dict)
  legalTypeSet1 = Set(["PROTEIN", "METABOLITE"])
  legalTypeSet2 = Set(["PROTEIN"])
  # check parameter setting
  rever = false
  ...
  if DS1SemanticChecking(senInfo[2][1], u2sDict, legalTypeSet1)  # reactants correct
    products = createProductsList(senInfo[2][1], s2uDict["_c"], s2uDict["_exc"])
...
BA
=#
function checkUptakeType(senInfo::Tuple, u2sDict::Dict, s2uDict::Dict)
  ...
  if semantics  # reactants correct
    products = createProductsList(senInfo[2][1], s2uDict["_exc"], s2uDict["_c"])
    if isdefined(senInfo[2], 2)  # have transporters
      if DS10SemanticChecking(senInfo[2][1], u2sDict, legalTypeSet1)  # unambiguous
        semantics2 = DS1SemanticChecking(senInfo[2][2], u2sDict, legalTypeSet2)
...
BA
=#
function checkReactType(senInfo::Tuple, conDict::Dict)
  legalTypeSet1 = Set(["PROTEIN", "METABOLITE", "MRNA", "GENE"])
  legalTypeSet2 = Set(["[]"])
  # check parameter setting
  rever = true
  ...
  if length(senInfo[2]) < 2
    println("no enough input for sentence $(senInfo[1])")
  else
    if length(senInfo[2]) > 2
...
IOBA
=#
function createProductsList(oriList::Array, oldTag, newTag)
  newList = []
  for arr in oriList
