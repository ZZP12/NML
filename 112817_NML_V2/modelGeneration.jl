  ...
  for tuple in preIRtuples
    ...
    if tuple[1] == "react"  # get rid of []
      ...
      elseif tuple[2][2][1].oriBioName == "[]"
        ...
        println("ERROR in \'react\', \'[]\' is not processed properly")
      end
      tmpRF.rnxName = name_r *"<"* tmpRF.rnxType *":"* name_c *">"* name_p
      push!(rnxList, tmpRF)
    elseif (tuple[1] == "uptake" || tuple[1] == "secrete" || tuple[1] == "bind" ||
      tuple[1] == "unbind" || tuple[1] == "phosphorylate" ||
      tuple[1] == "dephosphorylate" || tuple[1] == "catalyze")
      tmpRF = reactionForm()
      tmpRF.rnxType = tuple[1]
      name_r = ""
      name_c = ""
...
function sorting_species_list(unsorted_species_set::Set, mRNA_set::Set,
  ...
  elseif contains(sys2user["_exc"], "~")
    ...
    delimiter_com = "_"
    println("weired compartment tag $(sys2user["_exc"])")
  end
  ...
  elseif contains(sys2user["PROTEIN"], "~")
    ...
    delimiter_type = "_"
    println("weired type tag $(sys2user["PROTEIN"])")
  end
  # sort by type
  ...
  # merge results
  sorted_all_species_array = [sort(extracellular_metabolite_array);
