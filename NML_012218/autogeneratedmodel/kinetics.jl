function calculate_kinetics(X, data_dictionary)
	...
	rnx_rate_vector = zeros(5)
	rnx_rate_vector[1] = kcat[1]*X[1]/(MonodK["MonodK_1_met_A_e"]+X[1])
	rnx_rate_vector[2] = kcat[2]*X[6]*X[3]/(MonodK["MonodK_2_met_A_c"]+X[3])
	rnx_rate_vector[3] = kcat[3]*X[4]/(MonodK["MonodK_3_met_B_c"]+X[4])
	rnx_rate_vector[4] = kcat[4]*X[5]/(MonodK["MonodK_4_p_C_c"]+X[5])
	...
	# mRNA_E_c and p_E_c
	mRNA_E_c_p_C_c = W_value_dict["W_mRNA_E_c_p_C_c"] * X[5]^coop/((disassociation_constant_dict["KD_p_C_c"])^coop+X[5]^coop)
