...
function build_kinetics_buffer(all_species_dict::Dict{String, Int},
  ...
  kinetics = "function [rnx_rate_vector, TX_rate_vector, TL_rate_vector] = calculate_kinetics(X, data_dictionary)"
...
function build_data_dictionary_buffer(host_type::Symbol, all_species_array::Array,
  ...
  # data dictionary
  buffer = "function dataDictionary = generate_dataDictionary()"
  ...
  if host_type == :bacteria
    buffer *= replace(replace(include_constants_from_literature("txtl_constants_ecoli.jl","\t"), "#", "%"), r"(  | \t|\t\t)% ", "  ; % ")
  else
    buffer *= replace(include_constants_from_literature("txtl_constants_hl60.jl","\t"), "#", "%")
  end
