def calculate_kinetics(X, data_dictionary):
	...
	rnx_rate_vector = np.zeros(5)
	rnx_rate_vector[0] = kcat[0]*X[1]/(MonodK["MonodK~rnx1~met_A_e"]+X[1])
	rnx_rate_vector[1] = kcat[1]*X[6]*X[3]/(MonodK["MonodK~rnx2~met_A_c"]+X[3])
	rnx_rate_vector[2] = kcat[2]*X[4]/(MonodK["MonodK~rnx3~met_B_c"]+X[4])
	rnx_rate_vector[3] = kcat[3]*X[5]/(MonodK["MonodK~rnx4~p_C_c"]+X[5])
	rnx_rate_vector[4] = kcat[4]
	...
	# mRNA_E_c and p_E_c
	mRNA_E_c_p_C_c = W_value_dict["W~p_C_c~mRNA_E_c"] * X[5]**coop / ((disassociation_constant_dict["KD~p_C_c"])**coop+X[5]**coop)
	...
	TL_rate_vector["p_E_c"] = kcatTL * RIBOconc*X[7]/(saturationTL+X[7])
